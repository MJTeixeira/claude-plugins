---
name: godot
description: Any work in a Godot project (`project.godot` present) — read before running the godot CLI, tests, exports, or headless scene runs. A factory window has no editor and no eyes on a screen, so the headless invocations here are the only way it sees anything at all.
---
