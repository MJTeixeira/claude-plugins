...
export const runDoctor = (ctx) => {
  ...
  // 17. log retention must be a whole number of days — every window's sweep
  //     reads it, and a typo must never read as "delete more". Same stance as
  //     riskTiers: malformed turns the thing OFF and says so, rather than
  //     falling back to a default nobody wrote. Zero is not malformed (T-083):
  //     it is how an owner turns the sweep off, so it reads green and says so.
  if (cfg.logRetentionDays !== undefined) {
    const mode = retentionMode(cfg.logRetentionDays);
    check(mode === "invalid" ? "fail" : "ok", "log retention",
      mode === "on" ? `every window sweeps session logs older than ${cfg.logRetentionDays} days (never a window the surface has not acked)`
        : mode === "off" ? "logRetentionDays is 0 — the sweep is off and log/ keeps every session, by choice"
          : `logRetentionDays is ${JSON.stringify(cfg.logRetentionDays)} — expected a non-negative whole number of days (0 turns the sweep off); until fixed nothing is pruned and log/ grows forever`);
  }
