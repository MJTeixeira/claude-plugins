  ...
  if (fs.existsSync(path.join(project, ".git"))) {
    ...
    try {
      ...
      for (const rel of [".factory/config.json", ".factory/.env",
        ...
        // A tracked LINK (the meta worktree's log/plan.json link, pushed by
        // a pre-belt metadata commit) is not state: its target stays where
        // it is. existsSync follows it, so it would read as "still on disk"
        // below and stay tracked — and `git rm --cached` alone is undone by
        // the pathspec'd commit, which records the working tree. Drop the
        // dead link; the deletion then stages like any moved path.
        if (fs.lstatSync(path.join(project, rel), { throwIfNoEntry: false })?.isSymbolicLink()) fs.unlinkSync(path.join(project, rel));
...
if (mode === "mcp-server") {
  ...
  await runMcpServer({ project, eventsPath, stateDir: process.env.FACTORY_STATE_DIR, loadConfig });
}
...
// The checklist itself lives in doctor.mjs (runDoctor(ctx)); this wrapper
// hands it the loaded config and the shared helpers its checks read —
// shared because collectSchedulers, resolveCmd, the trust probes and
// isGitRepo also serve the schedule mode, the spawn preflight and prep.
const runDoctor = () => runDoctorChecks({
