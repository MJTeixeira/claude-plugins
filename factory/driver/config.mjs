...
export const buildConfig = (a) => ({
  ...
  tracker: "github", // = the forge's native tracker; "jira" routes needs-human + daily log to the Jira project in `jiraProject`, "discord" to threads in the channel `discordChannel` (or per-type `discordChannels` {questions,activity,digests}) scoped by `discordTag` with `discordOwnerId` as the trust anchor (all hand-set, like board)
