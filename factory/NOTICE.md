...
| `verify` | **ours** — the shipped `code4food-factory:verify` | Carried whole. Description drops its cross-reference to the old live verify; a §Finding the command section added, pointing at the repo's own config as the source of its commands; the Verify-tier reference reworded to stand alone now that the tier ladder is a live authoring rule. | knowledge-model swap |
| `godot`, `unity` | **ours** — `code4food-skillset` | The `.docs/index.md` § Commands pointer is dropped: the repo's own config (CI, run scripts, `project.godot` / `ProjectVersion.txt`) is the source, since a documented copy of a lookup goes stale and the config cannot. Godot's `.docs/mcp.md` pointer cut. Descriptions rewritten for a window with no editor open. | knowledge-model swap |
